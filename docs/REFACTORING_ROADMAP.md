...
Create a wrapper that makes the new engine compatible with the old `acs_engine.py` API so existing code doesn't break.
...
2. **Update Entry Points** (1 hour)
   - `acs_launcher.py` - Use new engine
   - `acs_ide.py` - Use new engine or wrapper
...
Refactor `acs_ide.py` (1450 lines) into MVC pattern with reusable components.
